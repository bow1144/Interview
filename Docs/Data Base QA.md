...
### 主键的自增id和uuid有什么区别？
* 自增id是从1开始的，uuid是一个128位的数字
* 不同表的id可能一样，而uuid是全球唯一的
* 自增id的性能开销小，而uuid没有规律，造成大量随机IO，性能开销大
* 自增id适合小型单一数据库，uuid适合分布式数据库

### 为什么自增ID更快一些，UUID不快吗，它在B+树里面存储是有序的吗？
* 自增id的下一条记录写在新的页中，数据按顺序加载，提升页面填充率，且寻址更快
* 每次产生的uuid无序，插入B+Tree的开销太大

### B+tree的叶子节点结构是什么？
* 由索引值和数据指针组成

### 你说你会MySQL，那它有哪些存储引擎？
* InnoDB：默认引擎，支持ACID事务，适用于高并发读写操作
* MyISAM：系统开销较低，不支持事务，行级锁，在并发方面有一定约束

### mysql和pgsql区别是什么？
* MySQL面向应用开发者，pgsql面向数据库开发者
* pgsql支持json、xml等现代应用程序功能，Mysql只支持json
* pgsql 执行复杂查询时表现良好
...
### 事务的MVCC是如何实现的？
* MVCC是多版本并发控制，由*读提交*进到*可重复读*
* *读提交*每次select之后产生`Read View`，*可重复读*实在启动事务时生成一个`ReadView`，保证了可重复读

### 事务怎么避免脏读？
* 从*读未提交*升级到*读已提交*

### InnoDB的四种隔离级别，你平常做项目和实习用的什么隔离级别？
* 默认级别，也就是可重复读
...
### B+Tree和BTree的比较？
* B+tree的非叶子节点只存放索引，有冗余节点，使得插入删除效率更高
* B+Tree更加矮胖，磁盘IO的次数更少
...
* 左或左右模糊匹配
* 查询中对索引列使用函数、表达式计算
* 索引是字符串，条件语句是数字，发生隐式转换，相当于对索引使用函数
* 没有按照最左匹配原则
* where语句中，or前面的是索引列，后面的不是

### 有一个用户表，属性有：用户唯一ID、性别，查询条件这两个属性都要用到，如何设置联合索引？
* 区分度大的字段放前面，区分度小的字段放后面
* 所以创建索引`(ID, gender)`
...
### MySQL中的Bin log作用是什么？
* Server层实现的日志，用于备份恢复，主从复制

### 了解SQL注入吗？
* 每次用户登录时，都会执行一个SQL语句，黑客通过构造恶意输入参数，去拼接SQL语句时篡改数据库信息
...
### 了解跳表吗？
* 时间复杂度接近`O(logn)`的类树形数据结构
* 由多个链表组成，由上到下链表节点越来越多，最后一个链表包含所有的数
* 增：在最底层加，上面随机增加
* 删：全删了
* 查：从上到下，类似二分
...
### 本地缓存和Redis缓存的区别？
* 本地缓存是将数据存储到本地服务器上，用内存作介质
* Redis缓存是分布式缓存，将数据存储到多个分布式节点
...
### Redis中的String如何存储，为什么不用C语言中的String？
* 字符串长度
* 分配的空间长度
* sds类型
* 字节数组

### Redis购物车为什么用Hash不用String？
* 结构化：key一般是String
* 节省内存
* 操作灵活
...
### 热Key是什么，怎么解决？
* 热Key是被频繁访问的Key，导致单个Key访问量过大影响性能
* 解决方法：
  - 内存淘汰机制，淘汰冷门的Key
  - 定时过期机制，防止长期占据内存
  - 对热点Key进行分片，减轻单个Key的压力
...
* AOF日志：每执行一条写命令，就追加写入到文件中，增量备份
* RDB快照：将某一时刻的内存数据写入磁盘中，一次全量备份
* 混合持久化理论：集成二者的优点
...
### 【重要】缓存雪崩、击穿、穿透和解决方法？
* **缓存雪崩**：某时间点**大量缓存失效**或Redis宕机，系统涌入后端；解决方法是合理设置缓存过期时间，分散缓存失效时间点
* **缓存击穿**：**某个热点缓存失效**，大量请求访问数据库；解决方法是互斥锁保证只有一个请求访问数据库、预热缓存，预先加载热点数据到缓存
* **缓存穿透**：数据在缓存、数据库中都不存在，导致数据库负载巨大。解决方法包括布隆过滤器

### 什么是布隆过滤器？原理是什么？
* 在用户请求到来时，业务线程确认缓存失效后，可以通过查询布隆过滤器快速判断数据是否存在，如果不存在，就不用通过查询数据库来判断数据是否存在
* 写入数据时用哈希函数标记，下次访问时只需查询布隆过滤器，如果没有被标记说明不在数据库中
