...
## 四、使用场景
* **计数器**：读写性能非常高，适合左频繁读写的计数器
* **缓存**：将热点数据放在内存中，保证缓存命中率
* **查找表**：例如DNS很适合放在Redis中
* **消息队列**：List是一个双向队列，可以用作消息队列
* **会话缓存**：实现高可用性以及可伸缩性
* **分布式锁**：用Redis自带的SETNX实现分布式锁

## 五、Redis与Memcached的区别
两者都是NoSQL，主要有以下区别：

* **数据类型**：Redis支持五种，Memcached仅支持字符串
* **数据持久化**：Redis支持RDB快照和AOF日志两种持久化，Memcached不支持持久化
* **分布式**：Redis支持分布式，Memcache不支持分布式
* **内存管理**：Redis可以将一些数据交换到磁盘，Memcached的数据一直在内存中

## 六、key的过期时间
* Redis可以设置每个key的过期时间，key过期后删除该键值对
* 对于散列表，只能为整个散列表设置过期时间

## 七、数据淘汰策略
可以为Redistribution设置内存最大使用量，内存超出后执行淘汰策略

|策略|描述|
|---|---|
|volatile-lru|已过期的最近最少淘汰（LRU）|
|volatile-ttl|集中挑选过期的淘汰|
|volatile-random|已经过期的任意淘汰|
|allkeys-lru|所有的最近最少（LRU）|
|allkeys-random|所有的任意淘汰|
|noeviction|禁止驱逐数据|

## 八、持久化
Redis是内存型数据库，为了保证数据在断电后不丢失，需要将内存中的数据持久化到硬盘中

### RDB持久化
创建快照，将某个时间点的所有数据存在硬盘上，系统崩溃后丢失最后一次快照后的数据

### AOF持久化
在一定操作（可调）后，将写命令同步到缓冲区、由OS决定什么时候同步到磁盘

## 九、事务
* 一个事务包含多个命令，服务器同时只执行一个事务
* 事务中的命令被一次性发送给服务器（流水线），可以减少客户端服务器通信次数，提升性能
* Redis最简单的事务实现方式是用`MULTI`和`EXEC`命令将事务包围
...
* 服务器通过套接字与客户端或其它服务器通信
* 用I/O多路复用程序监听多个套接字，将事件传送给文件事件分派器，分派器调用相应的命令请求处理器

![Redis文件事件](https://camo.githubusercontent.)

### 时间事件
* 时间事件是指服务器在给定时间点执行的操作
* 时间事件分为定时事件和周期性事件
* Redis将所有时间事件放在一个无序链表中，通过遍历链表找到操作

### 事件的调度与执行

![image](https://github.com/user-attachments/assets/cbd0a9c4-7e02-4956-b168-f81c1306253a)

## 十一、复制
* 通过`slaveof host port`使得一个服务器成为另一个服务器的从服务器
* 一个从服务器只能有一个主服务器

### 连接过程
1. 主服务器创建快照，从服务器接收快照并存储在缓冲区
2. 从服务器丢失所有旧数据，载入快照；并开始接收主服务器的写命令
3. 主服务器每执行一个写命令，就向从服务器发送相同的写命令

### 主从链
随着负载上升，主服务器无法很快更新所有从服务器。为了解决这个问题，创建一个**中间层**分担主服务器的复制工作

![image](https://github.com/user-attachments/assets/5bf3179e-7662-4e3b-b39e-91b14706097a)

## 十二、哨兵
哨兵（*Sentinel*）可以监听集群中的服务器，在主服务器下线时自动从从服务器中选举新的主服务器

## 十三、分片
分片是将数据划分为多个部分的方法，将数据存储到多台机器里面，可以获得线性级别的性能提升

* 范围分片：按id的范围分片，需要维护一张映射表，维护代价高
* 哈希分片：将key转换为一个数字并哈希取模
* 根据执行分片的位置：
  - 客户端分片
  - 代理分片
  - 服务器分片
...
### 文章信息
用哈希表的键值对存储文章信息，整个哈希表的key是文章id

### 点赞功能
* 对文章的`votes`字段加一
* 已投票集合：记录用户已经投票、防止用户点赞次数超过1
* 如果要求点赞期限为一周，可以为文章的**已投票集合**设定过期时间

### 文章排序
维护一个ZSet（有序集合）存储点赞文章排名
