...
### 动态数组有哪些？
* `Vector`：线程安全，一次扩容一倍
* `ArrayList`：线程不安全，一次扩容0.5倍

### HaskMap的实现原理
* JDK1.7之前，由数组和链表组成
* 1.8之后，使用红黑树实现，因为可以提高性能；但是在数据量很小的情况下还是会用索引

### HashMap的扩容条件是什么？
* 当前数据的数量大于阈值，并且出现哈希冲突
* 但Java8中只需要满肚第一个条件

### HashMap的put、get的过程
* 存储对象时，通过哈希函数计算哈希值，得到桶的位置，同时检查是否需要resize
* 获取对象时，通过hash函数计算桶的位置，如果由哈希冲突就通过链表查找

### HashMap一般用什么作Key，为什么？
* string
* 不可变，让hashcode的equals方法一致

### hashmap的扩容机制
* 将map容量扩充到两倍，然后从旧哈希表复制到新的哈希表中

### 说说Java中的List？
* `ArrayList`：动态数组，支持随机访问和快速插入删除
...
### 了解volatile吗？
保证了两个性质：   
* 可见性：多个线程访问一个变量时，一个线程修改变量、其余线程立刻看见
* 有序性：对于一个volatile写操作，执行在所有读操作之前，即**禁止指令重排序**

### volatile可以保证线程安全吗？
* 不能保证原子性，因此不能保证线程安全
* 在多线程情况下，多个线程同时操作会导致线程不安全，因此应该用`synchronize`等关键字

### volatile汇编是怎么实现的？
* 写操作之前插入StoreStore屏障，写操作后插入StoreLoad屏障
* 读操作之前插入LoadLoad屏障，读操作后插入LoadStore屏障

### 16.5 Java中线程池有哪些？
* ScheduledThreadPool：设置定期任务，支持周期性执行任务
* FixedThreadPool：固定线程数的线程池
* CacheThreadPool：缓存线程池，线程数几乎可以无限增加
* SignalThreadPool：会用唯一的线程执行任务

### 说一下线程池的常见配置
* 核心线程数量
* 最多线程数量
* keepAliveTime：空闲时间超过这个会被销毁
...
### 16.6 线程池淘汰策略有哪些？
* 线程池任务队列满了之后，会执行指定的拒绝策略来应对
* CallerRunsPolicy：使用调用线程池的线程执行被拒绝的任务
* AbortPolicy：直接抛出异常
* DiscardPolicy：不作处理，冷暴力
* DiscardOldestPolicy：抛出最老的任务
...
### Spring AOP的概念了解吗？
* 切面：切面是一个模块化的关注点，封装了横跨多个类的功能，如日志、性能监控、安全控制等。切面通常会包含一些特定的操作方法，并定义在哪些地方应用这些操作。
* 连接点（Join Point）：连接点是程序执行中的某个点，例如方法调用、方法执行前后等。AOP 可以在这些连接点处插入额外的功能。
* 通知（Advice）：通知是在切面中的实际操作，它指定了切面在特定连接点上需要执行的行为
* 切入点（Pointcut）：切入点是一个定义了切面应用的条件的表达式。它可以指定在哪些连接点上应用通知。通常，切入点通过方法签名来表达，指定哪些方法需要增强。
* 织入（Weaving）：织入是将切面应用到目标对象的过程。它可以在编译时、类加载时或运行时进行。

### Spring AOP是怎么实现的？
* 主要依赖动态代理，在运行时动态生成代理对象
* 基于JDK的动态代理
* 基于CGLIB的动态代理

### JDK、CGLIB两种动态代理有什么区别？
* JDK是委托机制，代理类（中介）和目标类实现了相同的接口，通过反射回调
* CGLIB是继承机制，代理类继承了目标类并重写了目标方法

### AOP与OOP的关系？
* AOP看作OOP的弥补
* AOP针对业务切面，面对处理过程的某个阶段，达到逻辑处理的隔离效果
...
### 介绍Spring Bean？
* Spring Bean 就是由 Spring 容器实例化、配置和管理的对象
* Bean是一个普通Java对象

### AOP实现有哪些注解？
* `@Aspect`定义切面
* `@Pointcut`定义切点
* `Before`,`After`：在方法执行前后通知
* `@Advice`通用通知类型
...
### 18.1 GC是什么？
* 垃圾收集的意思
* JVM提供的，自动检测对象是否需要内存收集，所以程序员不需要担心内存管理

### 18.2 说一下G1垃圾回收器？
* 关注最小时延的垃圾回收器，适合大尺寸堆内存的垃圾收集
* 相对于CMS有以下改进
  - 不会产生空间碎片
  - 可以设置停顿时间，防止雪崩
  - 利用并行与并发
